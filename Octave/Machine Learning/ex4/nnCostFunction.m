...
saveX = X;
X = [ones(m, 1) X];
mod = [ones(m, 1) sigmoid(X*Theta1')];
h = sigmoid(mod*Theta2');
...
Theta1_grad = ((Theta1_grad + delta2' * a1)/m) .+ ((lambda/m) * Theta1);
Theta2_grad = ((Theta2_grad + delta3' * a2)/m) .+ ((lambda/m) * Theta2);
